...
if ($_SERVER['REQUEST_METHOD'] == 'POST' || $debug) {
    ...
    //Get
    $cartRequest = new \YandexOld\Market\Models\PostCartRequest();
    $cartRequest->fromJson($postCartJson);
    $currency = $cartRequest->getCart()->getCurrency();
    /** @var YandexOld\Market\Models\Items $requestedItems */
    $requestedItems = $cartRequest->getCart()->getItems();

    //Ответ магазина на запрос от Яндекс.Маркета
    $postCartResponse = new \YandexOld\Market\Models\PostCartResponse();
    //Корзина
    $cart = new \YandexOld\Market\Models\CartResponse();

    //Товары в корзине
    $items = new \YandexOld\Market\Models\Items();
    /** @var YandexOld\Market\Models\Item $requestedItem */
    foreach ($requestedItems as $requestedItem) {
        //Информация о товаре в корзине.
        $item = new \YandexOld\Market\Models\Item();
    ...
    $deliveryOptions = new \YandexOld\Market\Models\DeliveryOptions();
    //Информация о доставке #1
    $deliveryOption1 = new \YandexOld\Market\Models\DeliveryOption();
    $deliveryOption1->setType(PartnerClient::DELIVERY_TYPE_PICKUP);
    $datesRange1 = new \YandexOld\Market\Models\DatesRange();
    ...
    $outlets = new \YandexOld\Market\Models\Outlets();
    $outlet1 = new \YandexOld\Market\Models\Outlet();
    $outlet1->setId(9);
    $outlet2 = new \YandexOld\Market\Models\Outlet();
    $outlet2->setId(10);
    $outlet3 = new \YandexOld\Market\Models\Outlet();
    $outlet3->setId(11);
    $outlets->add($outlet1);
    $outlets->add($outlet2);
    $outlets->add($outlet3);
    ...
    $deliveryOption2 = new \YandexOld\Market\Models\DeliveryOption();
    $deliveryOption2->setType(PartnerClient::DELIVERY_TYPE_DELIVERY);
    $datesRange2 = new \YandexOld\Market\Models\DatesRange();
